...
void ThreadRestrictions::AssertSingletonAllowed() {
  if (g_singleton_disallowed.Get().Get()) {
    CHROMIUM_LOG(FATAL) << "LazyInstance/Singleton is not allowed to be used on this "
...
void ThreadRestrictions::AssertWaitAllowed() {
  if (g_wait_disallowed.Get().Get()) {
    CHROMIUM_LOG(FATAL) << "Waiting is not allowed to be used on this thread to prevent "
               << "jank and deadlock.";
  }
}
